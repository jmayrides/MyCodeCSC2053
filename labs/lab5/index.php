...
foreach ($customers as $cust) {
  ...
  $id = $cust['id'];
  $url = "index.php?customer=$id";
  $name = $cust['name'];
  $university = $cust['university'];
  $city = $cust['city'];
  $sales = $cust['sales'];

  echo '<tr>';
  echo "<td class='mdl-data-table__cell--non-numeric'><a href=$url>$name</a></td>";
  echo "<td class='mdl-data-table__cell--non-numeric'>$university</td>";
  echo "<td class='mdl-data-table__cell--non-numeric'>$city</td>";
  echo "<td><span class='inlinesparkline'>$sales</span></td>";
  echo '</tr>';
...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  if (isset($_GET['customer'])) {
    ...
    $university = $requestedCustomer['university'];
    $address = $requestedCustomer['address'];
    $cityAndCountry = $requestedCustomer['city'] . ', ' . $requestedCustomer['country'];
?>
<!-- mdl-cell + mdl-card -->
...
that show below -->
<h4><?php echo "$name" ?></h4>
<?php echo "$university" ?><br>
<?php echo "$address" ?><br>
<?php echo "$cityAndCountry" ?>
